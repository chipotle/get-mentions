...
/** Type alias for webmention counter response object */
type Counter = {
    count: number;
    type: object[];
}

/** Type alias for mention response object */
type Mention = {
    type: string;
    name: string;
    children: object[];
 ...
 */
async function getCounts(url: string) {
    const counts = fetchResponse('count?target=' + url) as unknown
    return counts as Counter
}
 ...
 */
async function getAllMentions(pageSize, url: string) {
    const mentions = await fetchResponse(`mentions.jf2?target=${url}&sort-by=published&sort-dir=up&per-page=${pageSize}`) as Mention
